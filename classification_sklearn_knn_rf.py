...
# Using kNN with NLTK stop words
# instantiate learning model (k = 17)
# can test on different k
# 43: 82.67%
# 29: 83.67%
# 23: 85.17%
# 19: 86.50%
...
print('\n\nk-NN using nltk stemmed stop words:')
knn = KNeighborsClassifier(n_neighbors=19)
knn_stemmed = Pipeline([('vect', stemmed_count_vect),('tfidf', TfidfTransformer()),('knn', knn),])

# fitting the model
knn_stemmed.fit(description_train, category_train)
# predict the response
predicted_knn_stemmed = knn_stemmed.predict(description_test)

# evaluate accuracy
print_accuracy(np.mean(predicted_knn_stemmed == category_test))
knn_stemmed_matrix = data_analysis(category_test, predicted_knn_stemmed)

# plot the resulting data
visiulize(knn_stemmed_matrix, class_names, 'Normalized stemmed k-NN confusion matrix')


# Random forest
# best score: 89%%
print('\n\nRandom Forest using nltk stemmed stop words:')
rf = RandomForestClassifier(n_estimators = 450, oob_score = True, n_jobs = -1, random_state = 50)
rf_stemmed = Pipeline([('vect', stemmed_count_vect),('tfidf', TfidfTransformer()),('rf', rf),])
rf_stemmed = rf_stemmed.fit(description_train, category_train)

predicted_rf_stemmed = rf_stemmed.predict(description_test)

print_accuracy(np.mean(predicted_rf_stemmed == category_test))
rf_stemmed_matrix = data_analysis(category_test, predicted_rf_stemmed)

visiulize(rf_stemmed_matrix, class_names, 'Normalized stemmed Random Forest confusion matrix')
